...
public:
    explicit compat_error(std::string_view msg)
      : std::runtime_error(fmt::format("compat: {}", msg)) {}
};
...
template<typename T>
static void maybe_check(std::string name, json::Document& doc, bool& seen) {
    using type = corpus_helper<T>;
    if (type::checker::name == name) {
        vassert(!seen, "Duplicate check found: {}", name);
        seen = true;
...
static void check_types(
  type_list<Types...>, std::string name, json::Document& doc, bool& seen) {
    (maybe_check<Types>(name, doc, seen), ...);
}
...
static void check(json::Document doc) {
    ...
    auto seen = false;
    check_types(compat_checks{}, name, doc, seen);

    vassert(seen, "Type {} not found", name);
}
