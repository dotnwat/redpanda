...
struct test_config : public config::config_store {
    ...
    config::property<test_custom_aggregate> an_aggregate;
...
namespace std {
static inline ostream& operator<<(ostream& o, const test_custom_aggregate& c) {
...
namespace YAML {
template<>
struct convert<test_custom_aggregate> {
    using type = test_custom_aggregate;
