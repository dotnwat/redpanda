...
void admin_server::register_broker_routes() {
    ...
    register_route<superuser>(
      ss::httpd::broker_json::start_broker_drain,
      [this](std::unique_ptr<ss::httpd::request> req)
        -> ss::future<ss::json::json_return_type> {
          model::node_id id = parse_broker_id(*req);
          auto ec = co_await _controller->get_members_frontend()
                      .local()
                      .set_drain_mode(id, true);
          co_await throw_on_error(*req, ec, model::controller_ntp, id);
          co_return ss::json::json_void();
      });

    register_route<superuser>(
      ss::httpd::broker_json::stop_broker_drain,
      [this](std::unique_ptr<ss::httpd::request> req)
        -> ss::future<ss::json::json_return_type> {
          model::node_id id = parse_broker_id(*req);
          auto ec = co_await _controller->get_members_frontend()
                      .local()
                      .set_drain_mode(id, false);
          co_await throw_on_error(*req, ec, model::controller_ntp, id);
          co_return ss::json::json_void();
      });

    register_route<superuser>(
      ss::httpd::broker_json::start_local_drain,
      [this](std::unique_ptr<ss::httpd::request> req)
        -> ss::future<ss::json::json_return_type> {
    ...
    register_route<superuser>(
      ss::httpd::broker_json::stop_local_drain,
      [this](std::unique_ptr<ss::httpd::request> req)
        -> ss::future<ss::json::json_return_type> {
          co_await _controller->get_drain_manager().invoke_on_all(
    ...
    register_route<superuser>(
      ss::httpd::broker_json::get_local_drain,
      [this](std::unique_ptr<ss::httpd::request> req)
        -> ss::future<ss::json::json_return_type> {
          auto status
