...
admin_server::get_disk_stat_handler(std::unique_ptr<ss::http::request> req) {
    ...
    auto stat = co_await _storage_node.local().get_statvfs(type);
...
static json::validator make_disk_stat_overrides_validator() {
    const std::string schema = R"(
{
    "type": "object",
    "properties": {
...
admin_server::put_disk_stat_handler(std::unique_ptr<ss::http::request> req) {
    ...
    _storage_node.local().set_statvfs_overrides(type, overrides);
