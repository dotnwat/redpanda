...
ss::future<eviction_policy::schedule> eviction_policy::create_new_schedule() {
    ...
    const auto size = std::reduce(
      ...
      offsets.cend(),
      size_t{0},
      [](size_t acc, const shard_partitions& offsets) {
          return acc + offsets.partitions.size();
      });

    co_return schedule(std::move(offsets), size);
}
...
ss::future<> eviction_policy::install_schedule(schedule schedule) {
    co_await ss::parallel_for_each(schedule.shards, [this](auto& sched) {
        return _pm->invoke_on(sched.shard, [&sched](auto& pm) {
            for (const auto& group : sched.partitions) {
