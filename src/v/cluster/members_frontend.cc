...
ss::future<std::error_code>
members_frontend::set_drain_mode(model::node_id id, bool enabled) {
    auto leader = _leaders.local().get_leader(model::controller_ntp);
    if (!leader) {
        co_return errc::no_leader_controller;
    }
          ...
          .with_node_client<cluster::controller_client_protocol>(
            ...
            *leader,
            ...
            [id, enabled, timeout](controller_client_protocol cp) mutable {
                return cp.set_drain_mode(
                  set_drain_mode_request{.id = id, .enabled = enabled},
