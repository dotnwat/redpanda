...
ss::future<consensus_ptr> partition_manager::manage(
  ...
  std::vector<model::broker> initial_nodes) {
    ...
    /*
     * part of the node draining infrastructure. when a node is in a drianing
     * state new groups might be created since the controller will still be
     * active as a follower. however, if draining is almost complete then new
     * groups may not be noticed. marking as blocked should be done atomically
     * with adding the partition to the ntp_table index above for proper
