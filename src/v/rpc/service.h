...
struct service {
    template<typename Input, typename Output, typename Encoder>
      ...
      Func&& f) {
        return ctx.permanent_memory_reservation(ctx.get_header().payload_size)
          .then([f = std::forward<Func>(f), method_id, &in, &ctx]() mutable {
              return parse_type<Input, Encoder>(in, ctx.get_header())
                ...
                .then([method_id, &ctx](Output out) mutable {
                    const auto version = Encoder::version(ctx.get_header());
