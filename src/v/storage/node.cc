...
ss::future<> node::start() {
    ...
    if (test_disk_size_str) {
        _data_overrides.total_bytes = std::stoul(std::string(test_disk_size_str));
    }
...
ss::future<node::stat_info> node::get_statvfs(disk_type type) {
    const auto [path, overrides] = [&] {
        switch (type) {
        case disk_type::data:
            return std::tie(_data_directory, _data_overrides);
        case disk_type::cache:
            return std::tie(_cache_directory, _data_overrides);
        }
    }();
