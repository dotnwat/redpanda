...
void scheduler::request_immediate_finish(
  chunked_vector<std::pair<translator_id, size_t>> translators) {
    ...
    // `i` captures both priority and is a unique identifer. see
    // `on_resource_exhaustion` for how this property is used.
...
ss::future<> scheduler::main() {
    ...
    while (!_executor.as.abort_requested()) {
        ...
        vlog(
          datalake_log.trace,
          "scheduler tick,  memory_exhausted: {} immediate_finish: {}",
