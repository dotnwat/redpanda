...
nested_schema = Schema(
    ...
    NestedField(field_id=4, required=True, name="qux", field_type=ListType(
        element_id=5, element_required=True, element_type=StringType())),
    NestedField(field_id=6, required=True, name="quux",field_type=MapType(
        key_id=7, key_type=StringType(),
        value_id=8, value_type=MapType(
            key_id=9, key_type=StringType(),
            value_id=10, value_required=True, value_type=IntegerType()))),
    NestedField(field_id=11, required=True, name="location", field_type=ListType(
        element_id=12, element_required=True, element_type=StructType(
            NestedField(field_id=13, required=False, name="latitude", field_type=FloatType()),
            NestedField(field_id=14, required=False, name="longitude", field_type=FloatType())))),
    NestedField(field_id=15, required=False, name="person", field_type=StructType(
        NestedField(field_id=16, name="name", required=False, field_type=StringType()),
        NestedField(field_id=17, name="age", required=True, field_type=IntegerType()),
    ))
)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description=
        "Write an Apache Iceberg manifest file")
